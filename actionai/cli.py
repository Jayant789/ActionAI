...
from actionai.train import train
from actionai.predict import predict
...
def main():
    parser = argparse.ArgumentParser(
        description="ActionAI CLI - Train and test video classification models."
    )
    ...
    train_parser.add_argument(
        "--cfg", required=False, default=None, help="Model configurations"
    )
    ...
    predict_parser = subparsers.add_parser(
        "predict", help="Run a trained model on a video."
    )
    ...
    predict_parser.add_argument(
        "--video", required=True, help="Video to predict model on"
    )
    predict_parser.add_argument(
        "--save", required=False, action="store_true", help="Save results to file"
    )
    predict_parser.add_argument(
        "--display", required=False, action="store_true", help="Display results"
    )
    ...
    if args.command == "train":
        ...
        model_dir = (
            ...
            else config.get("DEFAULT", "model_dir", fallback=os.path.expanduser("~"))
        )
        ...
        result = train(
            data_dir, model_dir, window_size, learning_rate, epochs, batch_size
        )
        pprint(result)
